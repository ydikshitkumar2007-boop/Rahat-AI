...
export function CardHeader({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return <div className={`p-5 sm:p-6 border-b border-[#E5DED3] dark:border-[#323C4B] flex items-center justify-between gap-4 ${className}`}>{children}</div>;
}

export function CardTitle({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return <h3 className={`text-base font-semibold tracking-tight text-earth-900 dark:text-white font-sans ${className}`}>{children}</h3>;
}
